      ...
      <template v-if="!loading && !networkError && data">
        ...
        <section class="vct-card glow level-card">
          ...
          <div class="stats-row">
            <div class="stat-item">
              <div class="stat-num">
                {{ data.total_referrals || data.total_valid_referrals || 0 }}
              </div>
              <div class="stat-label">推荐人数</div>
            </div>
            <div class="stat-item">
              <div class="stat-num">
                {{ data.total_reward_cny || data.total_rewards_cny || 0 }}
              </div>
              <div class="stat-label">累积奖励(元)</div>
            </div>
            <div class="stat-item">
              <div class="stat-num balance">
                {{ data.balance_cny || data.cash_balance_cny || 0 }}
              </div>
        ...
        <section class="vct-card">
          ...
          <div class="link-tip">
            每邀请 1 位好友付费，你可得 {{ REFERRER_REWARD_CNY }} 元奖励
          </div>
        </section>
            ...
            <div v-for="r in records" :key="r.id" class="record-item">
              <div class="record-left">
                <div class="record-name">
                  {{ r.referee_name || r.invitee_nickname || "匿名用户" }}
                </div>
              ...
              <div
                class="record-reward"
                v-if="r.reward_cny || r.reward_amount_cny"
              >
                +{{ r.reward_cny || r.reward_amount_cny }}元
              </div>
            </div>
          </van-list>
        </section>
...
const nextLevel = computed(() => {
  ...
  const current =
    data.value.total_referrals || data.value.total_valid_referrals || 0;
...
async function doWithdraw() {
  ...
  try {
    ...
    if (data.value) {
      const key =
        data.value.balance_cny !== undefined
          ? "balance_cny"
          : "cash_balance_cny";
