<template>
  <div class="page">
    ...
    <!-- 当前订阅 -->
    <div
      v-if="mySub && !loading && !networkError"
      class="current-sub vct-card glow"
    >
      <div class="sub-header">
        ...
        <span v-if="mySub.expires_at" class="sub-expire">
          {{
            mySub.auto_renew
              ? "自动续费中"
              : "到期 " + formatTime(mySub.expires_at)
          }}
        </span>
      </div>
      <div class="sub-quota">
        本月扫描：{{ mySub.monthly_scans_used || 0 }} /
        {{ mySub.monthly_scans_quota || 3 }}
      </div>
      <van-button
        v-if="mySub.tier !== 'free'"
        size="small"
        plain
        type="primary"
        @click="renewSub"
    ...
    <div v-if="!loading && !networkError" class="product-list">
      ...
      >
        ...
        <div class="prod-price">
          ...
          <span v-if="p.original_price" class="original"
        ...
        <van-button
          ...
          :disabled="
            mySub?.tier === p.tier?.toLowerCase() && mySub?.status === 'active'
          "
          @click="buy(p)"
        >
          {{
    ...
    <!-- 空状态 -->
    <van-empty
      v-if="!loading && !networkError && products.length === 0"
      description="暂无可选产品"
    />
    ...
    >
      <div class="pay-info">
        ...
        <p v-if="orderResult?.order_no" class="pay-order">
