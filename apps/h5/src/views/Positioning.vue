<template>
  <div class="page">
    <van-nav-bar title="商业定位 BPS" left-arrow @click-left="router.back()" :border="false" />
    ...
    <div v-if="!loading && !networkError && !positioning" class="scan-section">
      <div class="vct-card intro-card">
        ...
        <div class="intro-desc">
          AI 分析你的变现潜力，推荐最合适的商业模式和成长路径，提供 12 个月路线图。
        </div>
      </div>
    ...
    <template v-if="positioning">
      ...
      <!-- 5 变现原型推荐 -->
      <section v-if="positioning.monetization_archetypes?.length" class="vct-card">
        ...
        <div class="monet-list">
          ...
          >
            ...
            <div class="monet-info">
              ...
              <div class="monet-fit" :class="fitCls(m.fit_score)">匹配度 {{ m.fit_score }}%</div>
            </div>
          </div>
        </div>
      </section>
...
const dimLabels: Record<string, string> = {
  market_size: '市场规模', competition: '竞争强度', monetization: '变现力',
  scalability: '扩展性', moat: '护城河', profitability: '盈利性',
}

const bmcLabels: Record<string, string> = {
  customer_segments: '客户细分', value_proposition: '价值主张', channels: '渠道通路',
  customer_relationships: '客户关系', revenue_streams: '收入来源', key_resources: '核心资源',
  key_activities: '关键业务', key_partners: '关键伙伴', cost_structure: '成本结构',
}

const dimensionScores = computed(() => {
  if (!positioning.value?.dimension_scores) return positioning.value?.scores || {}
...
function riskCls(r: string) {
  const map: Record<string, string> = { low: 'risk-low', medium: 'risk-mid', high: 'risk-high' }
...
async function doScan() {
  ...
  scanning.value = true
  try {
    const result = await positioningApi.scan({ description: description.value })
...
function resetScan() {
  ...
  description.value = ''
  descError.value = ''
  try { localStorage.removeItem(POSITIONING_CACHE_KEY) } catch { /* ignore */ }
}
...
.score-card {
  ...
  .match-title { font-size: 14px; color: var(--vct-text-2); margin-bottom: 16px; }
  ...
  .risk-tag {
    display: inline-block; margin-top: 16px; padding: 4px 14px; border-radius: 999px;
    font-size: 12px; font-weight: 600;
    &.risk-low { background: rgba(16,185,129,0.15); color: var(--vct-success); }
    &.risk-mid { background: rgba(245,158,11,0.15); color: var(--vct-warning); }
    &.risk-high { background: rgba(239,68,68,0.15); color: var(--vct-danger); }
  }
}

.monet-list { display: flex; flex-direction: column; gap: 10px; }
.monet-item { display: flex; gap: 12px; align-items: flex-start; }
.monet-rank {
...
.monet-info { flex: 1; }
.monet-name { font-size: 14px; font-weight: 600; }
.monet-desc { font-size: 12px; color: var(--vct-text-2); margin-top: 2px; }
.monet-fit { font-size: 11px; margin-top: 4px;
  &.high { color: var(--vct-success); }
  &.mid { color: var(--vct-warning); }
  &.low { color: var(--vct-text-3); }
}
...
.path-desc { font-size: 12px; color: var(--vct-text-2); margin-top: 2px; }
.path-revenue { font-size: 11px; color: var(--vct-success); margin-top: 4px; }

.roadmap-timeline { padding-left: 16px; border-left: 2px solid var(--vct-primary); }
.roadmap-item { padding: 10px 0; position: relative; }
.roadmap-month {
  font-size: 12px; font-weight: 600; color: var(--vct-primary); margin-bottom: 4px;
  &::before {
    content: ''; position: absolute; left: -21px; top: 14px;
    width: 8px; height: 8px; border-radius: 50%; background: var(--vct-primary);
    box-shadow: 0 0 8px rgba(245,158,11,0.5);
  }
}
.roadmap-goal { font-size: 13px; font-weight: 600; }
.roadmap-actions { font-size: 12px; color: var(--vct-text-2); margin-top: 2px; }
...
.bmc-grid {
  ...
  .bmc-cell {
    ...
    .bmc-key { font-size: 10px; color: var(--vct-primary); font-weight: 600; margin-bottom: 4px; }
    .bmc-val { font-size: 11px; color: var(--vct-text-2); word-break: break-all; }
  }
}
