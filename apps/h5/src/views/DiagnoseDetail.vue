<template>
  <div class="page">
    ...
    <div v-else-if="diag?.status === 'failed'" class="action">
      ...
      <van-button block @click="router.replace('/diagnose/submit')"
        >重新提交</van-button
      >
    </div>
  </div>
...
async function poll() {
  try {
    ...
    diag.value = data;
    if (
      [DIAGNOSIS_STATUS.COMPLETED, DIAGNOSIS_STATUS.FAILED].includes(
        data.status,
      )
    ) {
...
.circle {
  ...
  font-weight: 700;
  font-size: 22px;
  background: conic-gradient(var(--mfc-blue) 0deg, var(--mfc-bg-soft) 0deg);
  &.idle {
    color: var(--mfc-fg-3);
  }
  &.busy {
