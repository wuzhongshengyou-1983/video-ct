<template>
  <div class="page">
    <van-nav-bar title="复诊 — 对比改善" left-arrow @click-left="router.back()" :border="false" />

    <div v-if="origDiag" class="vct-card orig-card">
      <div class="orig-label">上次诊断</div>
      <div class="orig-url">{{ origDiag.video_url }}</div>
      <div class="orig-meta">{{ formatDate(origDiag.created_at) }} · 第 {{ origDiag.diagnosis_sequence ?? 1 }} 次</div>
    </div>
...
async function submit() {
  ...
  if (!url) { urlError.value = '请粘贴视频链接'; return }
  if (!VIDEO_URL_PATTERN.test(url)) { urlError.value = '仅支持抖音/快手/小红书/B站/视频号链接'; return }
...
.orig-card {
  ...
  .orig-label { font-size: 11px; color: var(--vct-text-3); margin-bottom: 6px; }
  .orig-url { font-size: 13px; color: var(--vct-text-1); word-break: break-all; margin-bottom: 4px; }
  .orig-meta { font-size: 11px; color: var(--vct-text-3); }
}
.form { margin: 0 16px; padding: 16px 0; }
.hint { margin: 16px 16px 0; font-size: 12px; color: var(--vct-text-2); line-height: 1.6; }
.submit-btn { margin: 20px 16px 8px; min-height: 50px; font-size: 17px; }
</style>
