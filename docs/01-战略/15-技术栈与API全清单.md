...
| 模块 | 选型 | 备选 |
|---|---|---|
| **大语言模型 API** | **硅基流动 SiliconFlow**（主力，100% 路由）：DeepSeek-V4-Flash / DeepSeek-V3.2 / DeepSeek-R1 / Qwen3-32B / Qwen2.5-72B / Qwen3.5-397B-A17B / Qwen3.5-4B · DeepSeek 官方 deepseek-chat（仅 fallback）| 阿里云百炼 / Claude API（备用） |
| **Embedding** | 硅基流动 bge-large-zh-v1.5（默认）/ Qwen3-Embedding-8B（pro） | text-embedding-3-large |
| **OCR** | **PaddleOCR**（开源）/ 硅基流动 PaddleOCR-VL | Tesseract（英文） |
| **ASR** | **Whisper Large v3**（自部署）/ 硅基流动 SenseVoiceSmall | 火山引擎 ASR |
| **视觉理解** | 硅基流动 Qwen3-VL-32B / PaddleOCR-VL | GPT-4o vision |
...
|---|---|---|---|
| **DeepSeek 官方** | deepseek-chat（已升级为 deepseek-v4-flash）/ deepseek-reasoner | 0.001 / 0.002 元 | **仅 fallback 兜底**，月 500 元 |
| **硅基流动 SiliconFlow** | deepseek-ai/DeepSeek-V4-Flash / DeepSeek-V3.2 / DeepSeek-R1 / Qwen3-32B / Qwen2.5-72B / Qwen3.5-397B-A17B / Qwen3-VL-32B / bge-m3 / bge-reranker-v2-m3 / PaddleOCR-VL 等 30+ 模型 | 0.0003-0.016 元 | **100% 主力路由** — chat + 推理 + 视觉 + Embedding + Rerank 全栈，月 5,000-8,000 元 |
| **阿里云百炼**（备用） | qwen-plus / qwen-max | — | 应急 |
| **阿里云百炼**（备用） | qwen-vl-max | — | 应急 |
| **Anthropic Claude API**（备用） | claude-sonnet-4-6 / claude-haiku-4-5 / claude-opus-4-7 | 0.003-0.075 USD | 应急/特殊场景 |
| **OpenAI API**（备用） | gpt-4o-mini / gpt-4o | 0.15 / 0.6 USD | 应急 |
| **MiniMax / Moonshot / 智谱 GLM**（备用） | 备选 | 价格相近 | 多供应商策略 |
...
（不含人力，人力详见 [08 实施路线图](08-实施路线图.md)）
